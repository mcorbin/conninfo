...
mod tests {
    ...
    fn parse_ipv4_file_test() {
        ...
        let e0 = &result[0];
        assert_eq!(if let ip::Ip::V4(val) = e0.local_address { val } else { 0 },(0x7F000001));
        assert_eq!(e0.local_port, 0x19);
        assert_eq!(if let ip::Ip::V4(val) = e0.remote_address { val } else { 99999 }, 0);
        assert_eq!(e0.remote_port, 0);
        assert_eq!(e0.uid, 0);
        assert_eq!(e0.connection_state, 0xA);

        let e1 = &result[1];
        assert_eq!(if let ip::Ip::V4(val) = e1.local_address { val } else { 99999 }, 0x7F000001);
        assert_eq!(e1.local_port, 0x8AE);
        assert_eq!(if let ip::Ip::V4(val) = e1.remote_address { val } else { 99999 }, 0);
        assert_eq!(e1.remote_port, 0);
        assert_eq!(e1.uid, 1000);
        assert_eq!(e1.connection_state, 0xA);

        let e2 = &result[2];
        assert_eq!(if let ip::Ip::V4(val) = e2.local_address { val } else { 99999 }, 0);
        assert_eq!(e2.local_port, 0x006F);
        assert_eq!(if let ip::Ip::V4(val) = e2.remote_address { val } else { 99999 }, 0x7F000203);
